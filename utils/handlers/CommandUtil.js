...
module.exports = async client => {
    (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async cmdFile => {
        ...
        if (!cmd.name) return Logger.warn(`Commande non-déclenché: ajoute un nom a ta commande.\nFichier -> ${cmdFile}`);

        if(!cmd.description && cmd.type != 'USER') return Logger.error(`Commande non-déclenché: ajoute une description a ta commande.\nFichier -> ${cmdFile}`);

        if (!cmd.category) return Logger.warn(`Commande non-déclenché: ajoute une catégorie a ta commande.\nFichier -> ${cmdFile}`);



        if (!cmd.permissions) return Logger.warn(`Commande non-déclenché: ajoute des permissions a ta commande.\nFichier -> ${cmdFile}`);

        cmd.permissions.forEach(permission => {
            if (!permissionList.includes(permission)) {
                return Logger.typo(`Event non-déclenché: ajoute une permission a ta commande. -> ${eventFile}`);
            }
        });
